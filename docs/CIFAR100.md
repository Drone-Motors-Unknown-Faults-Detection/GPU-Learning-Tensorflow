...
CIFAR-100 與 CIFAR-10 使用相同的圖片來源（32×32 彩色圖），但分類數從 10 擴充至 100，難度顯著提升。本模組在 CIFAR-10 ResNet 的基礎上加深一層（256→512），並採用 CosineDecay 與更豐富的資料增強，目標在 30 個 epoch 內達到 60%+ 測試準確率。

- **腳本**：`src/cifar100.py`
- **執行腳本**：`cifar100.sh`
- **日誌目錄**：`logs/CIFAR100/`
- **模型輸出**：`cifar100_resnet.weights.h5`
...
| 項目 | 內容 |
|------|------|
| 來源 | `tf.keras.datasets.cifar100`（自動下載至 `~/.keras/datasets/`） |
| 訓練集 | 50,000 張 |
| 測試集 | 10,000 張 |
| 圖片大小 | 32 × 32（RGB，3 通道） |
...
**訓練集**
```python
# RandomCrop + RandomHorizontalFlip + ColorJitter（同 CIFAR-10）
...
# Normalize（CIFAR-100 專屬統計值）
image = (image - mean) / std  # mean=(0.5071,0.4867,0.4408), std=(0.2675,0.2565,0.2761)
```
...
輸入 (32 × 32 × 3)
│
├─ Conv2D(64, 3×3) + BN + ReLU              →  32 × 32 × 64
│
├─ Layer1: 2× ResidualBlock(64→64,  s=1)    →  32 × 32 × 64
├─ Layer2: 2× ResidualBlock(64→128, s=2)    →  16 × 16 × 128
├─ Layer3: 2× ResidualBlock(128→256,s=2)    →  8 × 8 × 256
├─ Layer4: 2× ResidualBlock(256→512,s=2)    →  4 × 4 × 512   ← 新增層
│
...
| LR Scheduler | CosineDecay(decay\_steps=epochs × steps\_per\_epoch) |
| Dropout | 0.3（FC 前） |

### 為何改用 CosineDecay

StepLR 以固定步距階梯式降低 lr，在多分類任務中容易在降 lr 前後出現明顯的準確率震盪。CosineDecay 以餘弦曲線平滑地從 0.001 衰減至接近 0，整體訓練曲線更穩定，尤其在 epoch 15 ~ 30 的收斂階段效果更佳。
...
| LR Scheduler | 無 | CosineDecay | CosineDecay |
| 資料增強 | 無 | Crop + HFlip + ColorJitter | Crop + HFlip + ColorJitter |
